...
export class S2sWsRealtimeClient extends EventTarget {
  ...
  constructor(options) {
    ...
    /** @type {Record<string, any> | null} audio.cpp per-session config (TTS/ASR/
     *  LLM URLs + params) sent inside session.update as `session.audiocpp`. */
    this._appConfig = options.appConfig ?? null;
  ...
  _sendSessionUpdate() {
    ...
    // Only send instructions/voice when set, so an empty field can't clobber a
    // value carried in the audiocpp block below (the backend reads both).
    if (this.options.instructions) session.instructions = this.options.instructions;
    if (this.options.voice) session.audio = { output: { voice: this.options.voice } };
    // audio.cpp per-session config: TTS/ASR server URLs, model ids, voice ref,
    // LLM endpoint/key. The realtime backend reads this and reconfigures the
    // pipeline before the first turn.
    if (this._appConfig) session.audiocpp = this._appConfig;
  ...
  /** Update the audio.cpp pipeline config (TTS/ASR/LLM) on a live session.
   *  @param {Record<string, any>} cfg */
  updateAppConfig(cfg) {
    this._appConfig = { ...(this._appConfig || {}), ...cfg };
    this._send({
      type: "session.update",
      session: { type: "realtime", audiocpp: this._appConfig },
    });
  }
